			...
			<div class="box box-primary">
				...
				<div class="box-body">
				  <div class="row">
					<div class="col-md-4">
						...
						<input type="text" class="form-control custom-search" @keyup.enter="searchData" v-model="search" placeholder="Search parents, students">
					</div>
					</div>
					<div class="col-md-4">
						<div class="form-group">
			 ...
			 <div class="pagination-footer">
				 <div class="pagination-item">
					 <span>Per page:</span>
					 <vue-paginate :options="options" v-model="per_page" @input="fetchData(pagination = true)"></vue-paginate>
				 </div>
				 <div class="pagination-item">
...
import VuePaginate from '../../vendor/vue-paginate-select'
	export default{
		...
		created(){
			this.fetchData()
			this.$store.commit('title_bar_data', 'Students')
			document.title = 'Students | Invision'
			this.fetchSchools()
			this.fetchGrade()
		},
			methods: {
			fetchData(){
				var vm = this
				axios.get('/staff/students/', {headers: {Authorization: 'Token ' + Auth.getToken()}}).then(res => {
					vm.model = res.data
					this.renderList()
				})
			},
			...
			searchData(){
				var self = this
		       	const search = this.model.filter(function(mode){
		       		return mode.parent.toLowerCase().indexOf(self.search.toLowerCase())>=0 ||
